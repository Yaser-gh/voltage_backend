...
class FileRepository(BaseRepository[FileAsset]):
    ...
    async def get_by_project(self, project_id: UUID) -> list[FileAsset]:
        ...
        stmt = select(FileAsset).where(FileAsset.project_id == project_id, FileAsset.payment_id.is_(None), FileAsset.deleted_at.is_(None))
    ...
    async def get_by_payment(self, payment_id: UUID) -> list[FileAsset]:
        ...
        stmt = select(FileAsset).where(FileAsset.payment_id == payment_id, FileAsset.deleted_at.is_(None))
        return (self.session.scalars()).all()
